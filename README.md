  ...
  services:
    media-scripts:
      ...
      volumes:
        ...
        - /path/to/appdata/plex-meta-manager:/plex-meta-manager # optional - case sensitive
  ```
  ...
  docker run \
    ...
    -v /path/to/appdata/plex-meta-manager:/plex-meta-manager \
  ...
  1. **Mounting Folders**:
      - Make sure to mount the folder where you want the scripts to be downloaded to inside the container, as well as any additional folders such as your PMM directory.
      - For instance, on my system, I have these directories on my cache disk under the appdata share:
          ...
          volumes:
            ...
            - /mnt/user/appdata/plex-meta-manager:/plex-meta-manager
          ```
  ...
  1. **Adjusting Directories In .env File**:
      - There are a number of variables in your `.env` file that may need to be set to a full path. This path should relate to the location where you've mounted the folder *inside* your container.
        - For instance, based on the examples provided earlier, you may set:
          ```yaml
          PMM_CONFIG_DIR=/plex-meta-manager
          ASSET_DIR=/plex-meta-manager/assets
          ```
  ...
  1. The `runscript` command searches for the specified `script-name.py` within the `/config` folder (which is downloaded scripts' mount point inside the container).
